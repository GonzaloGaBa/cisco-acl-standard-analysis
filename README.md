...
* **Desvinculación de la interfaz:**
    R1# configure terminal
    R1(config)# interface serial 0/0/0
    R1(config-if)# no ip access-group 11 out
    R1(config-if)# exit

    * *¿Por qué?* Desvincular primero evita inconsistencias en la memoria del router y previene comportamientos inesperados durante el filtrado.

* **Eliminación de la regla global:**
    R1(config)# no access-list 11
    R1(config)# exit

* **Prueba de verificación post-cambio:**
